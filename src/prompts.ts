...
export async function chooseMySqlProfile(
  profiles: MySqlProfile[],
  defaultProfile?: string
): Promise<MySqlProfileChoice> {
  if (profiles.length === 0) {
    return { kind: "new" };
  }
...
}): Promise<MySqlConnection & { name: string; setDefault: boolean }> {
...
function validateDatabaseName(value: string): true | string {
...
export function filterTargetChoices(targets: KubeTarget[], term?: string): Array<{ name: string; value: KubeTarget }> {
...
export function filterJarCandidateChoices(candidates: JarCandidate[], term?: string): Array<{ name: string; value: string }> {
  const keyword = term?.trim().toLowerCase();
  return candidates
