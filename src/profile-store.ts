...
export async function readProfiles(filePath = defaultProfilesPath()): Promise<ProfilesFile> {
  ...
  try {
    ...
    throw new Error(`读取 profile 配置失败：${(error as Error).message}`);
  }
}

export async function writeProfiles(profilesFile: ProfilesFile, filePath = defaultProfilesPath()): Promise<void> {
  await fs.mkdir(path.dirname(filePath), { recursive: true, mode: 0o700 });
...
export async function setDefaultProfile(name: string, filePath = defaultProfilesPath()): Promise<void> {
  const config = await readProfiles(filePath);
...
export async function getProfile(name: string, filePath = defaultProfilesPath()): Promise<SavedProfile | undefined> {
  const config = await readProfiles(filePath);
  return config.profiles.find((profile) => profile.name === name);
}

export function chooseDefaultProfile(config: ProfilesFile): SavedProfile | undefined {
