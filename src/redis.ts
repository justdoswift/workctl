...
export type RedisOperation =
  ...
  | { action: "scan"; pattern: string }
  | { action: "custom"; command: string };
...
export function isRedisTarget(target: KubeTarget): boolean {
  return target.name.toLowerCase().includes("redis");
}

export function redisTargetPriority(target: KubeTarget): number {
...
export function sortRedisTargets(targets: KubeTarget[]): KubeTarget[] {
...
export function autoRedisTarget(targets: KubeTarget[]): KubeTarget | undefined {
  return targets.length === 1 ? targets[0] : undefined;
}

export function formatRedisTargetChoice(target: KubeTarget): string {
...
export function defaultRedisHostForTarget(target: KubeTarget): string | undefined {
...
export function describeRedisOperation(operation: RedisOperation): string {
  switch (operation.action) {
    case "ping":
      return "PING";
    case "info":
...
export function redactRedisPassword(value: string, password?: string): string {
  if (!password) {
    return value;
  }
